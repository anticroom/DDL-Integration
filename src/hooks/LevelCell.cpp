...
class $modify(DDLLevelCell, LevelCell)
{
    ...
    void loadFromLevel(GJGameLevel *level)
    {
        ...
        for (auto type : {DDLIntegration::ListType::DDL, DDLIntegration::ListType::DCL, DDLIntegration::ListType::DVL})
        {
            ...
            if (!DDLIntegration::isLoaded(type))
            {
                if (!s_fetching[index])
                {
                    s_fetching[index] = true;
                    DDLIntegration::loadLevels(type, s_listeners[index], [index]
                                               { s_fetching[index] = false; }, [index](int)
                                               { s_fetching[index] = false; });
                }
                continue;
            }

            for (auto const &lvl : DDLIntegration::levels(type))
            {
                if (lvl.id != levelID)
                    continue;
                rankStrings.push_back(fmt::format("#{} {} ({:.3f} pts)",
                                                  lvl.position, DDLIntegration::listName(type), DDLIntegration::calculateScore(lvl.position, type)));
            ...
            if (diffNode)
            {
                ...
                if (ring && face && fakeDDLRateFace)
                {
                    ...
                    if (bestRank <= 3)
                    {
                        ...
                        if (bestRank == 1)
                            fakeDDLRateLevel->m_isEpic = 3;
                        else if (bestRank == 2)
                            fakeDDLRateLevel->m_isEpic = 2;
                        else if (bestRank == 3)
                            fakeDDLRateLevel->m_isEpic = 1;
    ...
    void addRank(const std::vector<std::string> &ranks, int bestRank, DDLIntegration::ListType bestSource)
    {
        ...
        int legacyThreshold = DDLIntegration::getLegacyCutoff(bestSource);
